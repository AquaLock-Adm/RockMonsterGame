...
#region Main Functions
	...
	public void Setup(BattleSystem BS){
		...
		if(BattleSystem.Player.GetWeapon().actionsPerRound < 5) this.ActionQueue.heatChargeDone = true;
		SetupButtonTexts();
		LoadMainMenu();
	}
	...
	private void LoadBlocksMenu(){
		List<Action> BlocksList = BattleSystem.Player.GetWeapon().Blocks;
...
#region Helper Functions
	...
	private void NormalAttackCastInputs(){
		...
		if(BattleSystem.defendModeActive) BattleSystem.CastBlock(abilityIndex);
		else BattleSystem.CastAttack(abilityIndex);
	}
