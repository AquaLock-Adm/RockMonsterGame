...
public class WaveRandomizer : MonoBehaviour
{
    ...
    public List<EnemySettings> GetEnemySettingsList(){
        ...
        for(int enemyI = 0; enemyI < spawnWaveSize; enemyI++){
            ...
            if(stageIndex < 2){
                if(!RandomShieldInFirstTwoEnemiesCheck(res, RandomEnemySettings)){
                    enemyI--;
                    retries++;
    ...
    private bool RandomShieldInFirstTwoEnemiesCheck(List<EnemySettings> CurrentEnemySettingsList, EnemySettings Settings){
        if(CurrentEnemySettingsList.Count > 2) return true;

        foreach(List<ShieldMode> defenseMode in Settings.DefensiveModes[0]){
            if(defenseMode.Contains(ShieldMode.RANDOM)) return false;
        }
        ...
        Difficulties:
            1:
                if shieldCount >= 3
                 ...
                 max Attack Sequence = 1;
                else
                 hp: 10 <-> 30
                 max AttackSequence = 1;
    ...
    private EnemySettings SetupStageTwoBoss(){
        ...
        bossSettings = InitAttackLibrary(bossSettings, 3, 3, 4);
    ...
    private EnemySettings RandomizeSingleStageTwoEnemy(int shieldCount, int defensiveModeCount){
        ...
        */
        ...
        if(useRandomShield){
            ...
            Res = InitAttackLibrary(Res, 2, 3, 2);
        }else{
            Res.baseKillPrice = 24;
            Res.maxKillPrice = 2500;
            Res = InitAttackLibrary(Res, 3, 3, 2);
        }
