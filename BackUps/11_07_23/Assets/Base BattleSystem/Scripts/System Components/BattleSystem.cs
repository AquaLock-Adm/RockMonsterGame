...
#region SetupEnemySpawner Functions
    private void SetupEnemySpawner(){
        ...
        EnemySettings FirstEnemyOfWave;

        if(this.useWaveScript) {
            EnemySettingsList = this.WaveScript.GetEnemySettingsList();
            FirstEnemyOfWave = this.WaveScript.GetFirstEnemyOfWave();
        }else {
            EnemySettingsList = this.WaveRandomizer.GetEnemySettingsList();
            FirstEnemyOfWave = this.WaveRandomizer.GetFirstEnemyOfWave();
        }

        if(!this.inBossWave || (this.inBossWave && this.afterFinalWave)) EnemySettingsList.Insert(0, FirstEnemyOfWave);
...
#region Round Start Functions
    public void NextWave(){
        ...
        if(this.afterFinalWave || (!this.afterFinalWave && !this.inBossWave)) this.ActionQueue.heatChargeDone = false;
        this.ActionQueue.heatChargeDone = false;
        this.attackRushUsed = false;
        this.BattleMenuHandler.LoadMainMenu();
        SetupEnemySpawner();
        this.EnemySpawner.SpawnNextEnemy();
        if(this.inBossWave && !this.afterFinalWave) {
    ...
    public int GetNextWaveSize(){
        return this.WaveRandomizer.GetNextWaveSize()+1;
    }
#endregion
...
#region Input Functions during Player Turn 
    ...
    public virtual async void PassRound(){
        ...
        if(this.state == BattleState.START) ActionQueue.DisableHeatForOneRound();
