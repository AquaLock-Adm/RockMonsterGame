...
#region overrides
    ...
    public void Heal(int healAmount){
        ...
        if(this.deathTriggered) this.deathTriggered = false;
        UpdateHUDElements();
    }
    ...
    private async void EnemyDOTLoop(){
        if(this.Weapon.enemyDamagePerSecond <= 0) return;
        float dotPerTick = (float)this.Weapon.enemyDamagePerSecond / ( 1000.0f/(float)this.dotTickTime_ms ); 
    ...
    private void CalcLifeDrainPerTick(){
        this.lifeDrainPerTick = (float)((float)this.Weapon.healthCostPerSecond / (1000.0f/(float)this.lifeDrainTickTime_ms)); 
    }
