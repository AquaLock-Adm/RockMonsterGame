...
#region SetUp Functions
    ...
    public void SetBattleUnits(){
        ...
        if(!this.enemySet)this.Enemy = this.BattleSystem.Enemy;
    }
    public void BaseDamageCalculation(){
        ...
        for(int stringI = 0; stringI < this.comboList.Length; stringI++){
            switch(this.comboList[stringI]){
                case 'L':
                    this.baseDamage += this.BattleSystem.CalcUnitDamage(this.BattleSystem.Player);
                    this.maxBaseDamage += this.BattleSystem.Player.attackMax;
                    this.minBaseDamage += this.BattleSystem.Player.attackMin;
                break;

                case 'H':
                    this.baseDamage += (int)Mathf.Round(this.BattleSystem.CalcUnitDamage(this.BattleSystem.Player)*this.BattleSystem.heavyAttackMultiplier);
                    this.maxBaseDamage += (int)Mathf.Round(this.BattleSystem.Player.attackMax * this.BattleSystem.heavyAttackMultiplier);
                    this.minBaseDamage += (int)Mathf.Round(this.BattleSystem.Player.attackMin * this.BattleSystem.heavyAttackMultiplier);
                break;

                case 'S':
                    this.baseDamage += (int)Mathf.Round(this.BattleSystem.CalcUnitDamage(this.BattleSystem.Player)*this.BattleSystem.specialAttackMultiplier);
                    this.maxBaseDamage += (int)Mathf.Round(this.BattleSystem.Player.attackMax * this.BattleSystem.specialAttackMultiplier);
                    this.minBaseDamage += (int)Mathf.Round(this.BattleSystem.Player.attackMin * this.BattleSystem.specialAttackMultiplier);
                break;
    ...
    public void BaseApCalculation(){
        ...
        for(int stringI = 0; stringI < this.comboList.Length; stringI++){
            switch(this.comboList[stringI]){
                case 'L':
                    this.apCost += this.BattleSystem.Player.Weapon.lightAttackCost;
                break;

                case 'H':
                    this.apCost += this.BattleSystem.Player.Weapon.heavyAttackCost;
                break;

                case 'S':
                    this.apCost += this.BattleSystem.Player.Weapon.specialAttackCost;
                break;
    ...
    public void SetAccuracy(){
        ...
        this.accuracy = this.BattleSystem.Player.Weapon.GetComboAccuracy(this.comboLevel);
    }
#endregion
    ...
    public async Task<int> NormalExecute(){
        ...
        if(!this.alreadyChecked){  // checks only for first time in case of AOE or MultiHit effects
            ...
            if(!await this.CheckMana()) return 0;
...
#region Execute Attack Functions
    ...
    private async Task GetManaGain(){
        await this.Player.DepleteMana(-this.manaGain);
        this.manaGain = 0;
    }
    ...
    public void PrintAction(){
        ...
        s +=      "Combo Level: "+this.comboLevel.ToString()+"\n"+
                    ...
                    float cElemWeakf = 1.0f;
                    if(this.element != SpellElement.none) cElemWeakf = this.BattleSystem.spellStrongMultiplier;
                    s += "Max Damage = "+((int)Mathf.Round(
                        ...
                        *(this.BattleSystem.critMultiplier+this.bonusCritFactor+this.Player.Weapon.bonusCritDamagef))).ToString() +"\n";
                    ...
                    if(this.executedCrit){
                        this.executedCrit = false;
                        cCritf = this.BattleSystem.critMultiplier;
...
public class Light : Action
{
    ...
    public override void Setup(){
        ...
        this.manaCost = 0;
        this.crit = this.BattleSystem.Player.crit;
        this.SetAccuracy();
    ...
    public override void Setup(){
        ...
        switch(this.element){
            ...
            case SpellElement.FIRE:
                this.baseDamage =  90 + (this.BattleSystem.Player.level * 5);
                this.manaCost = 8;
                this.apCost = 7;
            break;

            case SpellElement.WATER:
                this.baseDamage =  100 + (this.BattleSystem.Player.level * 7);
                this.manaCost = 12;
                this.apCost = 6;
            break;

            case SpellElement.AIR:
                this.baseDamage =  50 + (this.BattleSystem.Player.level * 9);
                this.manaCost = 12;
                this.apCost = 3;
            break;
