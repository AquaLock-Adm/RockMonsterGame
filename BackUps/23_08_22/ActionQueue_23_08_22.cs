/*
LOG: Version before adding the constant input system. where Player inputs can be done during the execution of the actionqueue
	- in this version the player has to wait for the actions in action q to play out before queueing up new actions
*/
...
public class ActionQueue : MonoBehaviour
{
	...
	public async Task ExecuteAllActions(){
		...
		while(Actions.Count > 0 && !stopQueue) {
			if(!pauseQueue){
				...
				Action CurrentAction = DequeueFirstAction();
	...
	public Action DequeueFirstAction() {
		if(Actions.Count > 0){
			ActionText.text = "";
			if(Actions.Count == ComboList.Count) {
